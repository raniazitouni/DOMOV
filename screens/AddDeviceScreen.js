...
const ChangeCredentialsScreen = () => {
	...
	const handleUpdateCredentials = async () => {
		setIsLoading(true);
		await Api.post(`update_credentials`,{
			...
			if (response.ok) {
				...
				Alert.alert(
					"Error",
					`Failed to update credentials. Server response: ${responseBody}`
				);
			}
		})
		.catch(error => {
		...
		<ScreenWrapper>
			<ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "center"}}>
				<View style={tw`mx-5 flex flex-col`}>
					...
					<Text style={[ tw`ml-3 text-18px mb-5 mt-7 pb-1`, { fontFamily: "Inter-Regular", color: colors.maingrey}]}>
						...
						*/}
						...
						<View style={styles.views}>
							<TouchableOpacity
								...
								disabled={
									isLoading || !isPasswordValid(newPassword)
								}
							>
